...
testCM("doubleScrollbar", function (cm) {
    ...
    is(
        wrap.offsetWidth -
            byClassName(wrap, "CodeMirror-lines")[0].offsetWidth <=
            scrollbarWidth * 1.5,
    );
});
...
testCM(
    ...
    {
        mode: "text/html",
        value: "0123456789abcde0123456789",
        lineWrapping: true,
    },
    ie_lt8 || opera_lt10,
);
...
testCM(
    ...
    function (cm) {
        ...
        is(
            cursorBox.bottom <
                editorBox.top + cm.getScrollerElement().clientHeight,
            "bottom line visible",
        );
    },
    { lineNumbers: true },
...
testCM(
    ...
    function (cm) {
        ...
        eqCharPos(
            cm.coordsChar({ left: p0.left + 5, top: p0.top + 5 }),
            Pos(0, 0),
        );
        eqCharPos(
...
testCM(
    ...
    function (cm) {
        ...
        for (var i = positions.length - 2; i >= 0; i--) {
            ...
            eqCharPos(
                cm.getCursor(),
                i == 2 ? Pos(0, 6, "before") : positions[i],
            );
        }
    },
    { value: "  foo +++  \n  bar" },
...
testCM("lineWidgetChanged", function (cm) {
    ...
    w0.node.style.lineHeight =
        w150.node.style.lineHeight =
        w300.node.style.lineHeight =
            ...
            for (i = steps - 1; i >= 0; --i) {
                ...
                if (!(lineBreaks[i] == "n" || lineBreaks[i + 1] == "w")) {
                    ...
                    eq(
                        ...
                        "In step " +
                            i +
                            ", cursor is not at the same line anymore",
                    );
                } else {
                    is(coords.left > prevCoords.left, i);
                    is(coords.top < prevCoords.top, i);
...
testCM(
    ...
    function (cm) {
        ...
        is(
            byClassName(cm.getWrapperElement(), "CodeMirror-selected").length >=
                3,
        );
    },
...
testCM(
    "should have translate=no attribute",
    function (cm) {
